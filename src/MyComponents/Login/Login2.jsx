...
const Login2 = () => {
  ...
  const handleLogin = async()=>{
    const url = "https://mr-dinesh-doctor-patient-backend.vercel.app/api/v1/pharmacy/signIn";
    try{
      const {data} = await axios.post(url, {
        email, password
      })
      localStorage.setItem("ptoken",data?.accessToken);
      navigate("/total-medicine");
    }catch(err){
      ...
      <section class="flex justify-center shadow-2xl items-center h-screen bg-[#e0e7e8]">
        <div class="max-w-md w-full bg-white rounded p-6 space-y-4">
          <div class="mb-4">
            ...
            <h2 class="text-xl font-bold">to Pharmacist Panel</h2>
          </div>
          <div>
            <input
              value={email}
          ...
          <div>
            ...
            <p onClick={()=>navigate("/pharma-registration")} style={{cursor:"pointer"}}>Sign Up as New User</p>

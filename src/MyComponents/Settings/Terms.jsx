...
const Terms = () => {
  ...
  const getTerms = async()=>{
    const url = "https://mr-dinesh-doctor-patient-backend.vercel.app/api/v1/static/getTerms";
    try{
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const handleUpdate = async()=>{
      const url = `https://mr-dinesh-doctor-patient-backend.vercel.app/api/v1/static/terms/${id}`;
      try{
        console.log(terms);
        const {data} = await axios.put(url, {terms}, {
    ...
    const handleCreate = async()=>{
      const url = "https://mr-dinesh-doctor-patient-backend.vercel.app/api/v1/static/createTerms";
      try{
        console.log(terms);
        const {data} = await axios.post(url, {terms}, {
          headers : {Authorization : `Bearer ${token}`}
        })
        console.log(data);
        getTerms();
        ...
        <Modal.Body>
          <div className="terms1">
            <div className="terms2">
              ...
              <input type="text" placeholder="type here....." onChange={(e)=>setT(e.target.value)} />
            </div>
            {isUpdate ? <button className="bg-[#6e39eb] text-white p-2 rounded-lg text-sm"
                ...
                {terms?.map((ele, i) => (
                  <>
                    <tr className="divider " style={{ textAlign: "center" }}>
                      <td className="w-68">{ele?.terms}</td>
                      <td class="flex  justify-center items-center space-x-4 h-16 w-18 py-3 text-gray-900">
                        <MdEdit
                          onClick={() => { setUpdate(true); setId(ele?._id); setModalShow(true)}}
